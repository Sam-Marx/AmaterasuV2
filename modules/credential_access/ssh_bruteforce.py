...
'''
SSHBRUTEFORCE MODULE NEED TO BE
...
class SSHBruteforce(cmd2.Cmd):
	def __init__(self):
		...
		self.metadata = {'Description'	: 'Bruteforce SSH services.',
		...
		self.prompt = 'amaterasu[credential_access/ssh_bruteforce]> '
	...
	def bruteforce(self, lock, target: str, username: str, password: str, verbose: Optional[bool] = False) -> None:
		with lock:
			try:
				...
				connect = ssh.connect(target, 22, username, password)
